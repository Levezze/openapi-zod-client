...
function getZodSchema(_ref) {
  ...
  if (schema.allOf) {
    ...
    var withImplicitRequired = (options === null || options === void 0 ? void 0 : options.withImplicitRequiredProps) === true;
    var hasEmptyRequiredInParent = Array.isArray(schema.required) && schema.required.length === 0;
    var hasEmptyRequiredInAllOf = schema.allOf.some(function (item) {
      return !isReferenceObject(item) && Array.isArray(item.required) && item.required.length === 0;
    });

    // Only apply .partial() for empty required arrays when using implicit required mode
    var shouldApplyPartialForEmptyRequired = withImplicitRequired && (hasEmptyRequiredInParent || hasEmptyRequiredInAllOf);
    var finalCode = "".concat(first.toString(), ".").concat(rest);

    // Only apply .partial() in implicit-required mode with empty required arrays
    return code.assign(shouldApplyPartialForEmptyRequired ? "".concat(finalCode, ".partial()") : finalCode);
  }
